#!/usr/bin/env bash
# Regression check for lesson 0001: no hook/script may pipe data into a command
...
if grep -rnE '\|[[:space:]]*(python3?|bash|sh|node)[^|]*<<' \
  ...
  echo "pipe-into-heredoc pattern found (see lesson 0001) - the piped stdin is silently lost"
  exit 1
fi
exit 0
